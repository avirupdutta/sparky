...
export default defineContentScript({
  ...
  main(ctx) {
    ...
    ctx.addEventListener(document, "focusin", handleFocusIn, { capture: true });
    ctx.addEventListener(document, "input", handleInput, { capture: true });
    ctx.addEventListener(document, "keydown", handleKeyDown, { capture: true });
    ctx.addEventListener(document, "scroll", positionGhost, { capture: true });
    ctx.addEventListener(window, "resize", positionGhost);
    ...
    function requestCompletion() {
      ...
      void chrome.runtime
        .sendMessage({ type: "completion/request", payload })
        .then((response: BackgroundToContentMessage) => {
